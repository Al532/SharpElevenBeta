...
export function createChartSheetRenderer({
  ...
  function applyOpticalPlacements() {
    ...
    document.querySelectorAll('.chart-bar-body').forEach((barBodyEl) => {
      const collisionScale = getBarBodyCollisionScale(/** @type {HTMLElement} */ (barBodyEl));
      if (collisionScale >= 0.995) return;
      const currentFontPx = parseFloat(getComputedStyle(/** @type {HTMLElement} */ (barBodyEl)).fontSize);
      if (!currentFontPx) return;
      /** @type {HTMLElement} */ (barBodyEl).style.fontSize = `${(currentFontPx * collisionScale * textScaleCompensation).toFixed(2)}px`;
    });
