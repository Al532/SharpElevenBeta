...
function renderFilterChips(host: HTMLElement | null, key: ChartManageFilterKey, values: ChartManageFilterOption[]) {
  ...
  const buttons = [
    ...
    const button = createButton(label, 'library-filter-chip');
...
function renderManageCharts() {
  ...
  if (dom.manageLibrarySummary) {
    ...
    if (shouldSummarize) {
      ...
      row.className = 'home-list-link home-chart-entry library-summary-row';
      const summaryButton = document.createElement('button');
      summaryButton.type = 'button';
      summaryButton.className = 'library-summary-open';
  ...
  for (const chartDocument of documents) {
    ...
    const row = document.createElement('div');
    row.className = 'home-list-link library-chart-row';
    ...
    content.className = 'library-chart-link';
...
function renderSetlists() {
  ...
  for (const [setlistIndex, setlist] of currentSetlists.entries()) {
    ...
    const content = document.createElement('div');
    content.className = 'library-setlist-open';
    content.append(
    ...
    childList.className = 'home-list library-setlist-chart-list';
    ...
    setlist.items.forEach((setlistItem, index) => {
      ...
      childRow.className = 'home-list-link library-chart-row library-setlist-chart-row';
      ...
      childContent.className = 'library-chart-link';
...
function renderSetlistMenu(setlist: ChartSetlist): HTMLElement {
  ...
  menu.className = 'library-setlist-menu';
  if (renamingSetlistId === setlist.id) {
    const input = document.createElement('input');
    input.className = 'library-text-input library-setlist-menu-input';
    input.type = 'text';
    input.value = setlist.name;
    input.placeholder = 'Setlist name';
    const saveButton = createButton('Validate', 'library-small-action');
    const cancelButton = createButton('Cancel', 'library-small-action');
  ...
  const playButton = createButton('Play', 'library-small-action');
  playButton.addEventListener('click', () => {
    const targetUrl = new URL('./chart/index.html', window.location.href);
    targetUrl.searchParams.set('setlist', setlist.id);
    window.location.assign(targetUrl.toString());
  });
  const renameButton = createButton('Rename', 'library-small-action');
  renameButton.addEventListener('click', () => {
    renamingSetlistId = setlist.id;
    pendingDeleteSetlistId = '';
    renderSetlists();
  });
  const deleteButton = createButton(pendingDeleteSetlistId === setlist.id ? 'Confirm delete' : 'Delete', 'library-small-action library-danger');
...
function renderSetlistItemMenu(setlistId: string, itemIndex: number): HTMLElement {
  const menu = document.createElement('div');
  menu.className = 'library-setlist-menu library-setlist-item-menu';
  const deleteButton = createButton('Delete from setlist', 'library-small-action library-danger');
...
function renderCreateSetlistForm(): HTMLElement {
  const item = document.createElement('li');
  const row = document.createElement('div');
  row.className = 'library-setlist-menu library-create-setlist-row';
  const input = document.createElement('input');
  input.className = 'library-text-input library-setlist-menu-input';
  input.type = 'text';
  input.placeholder = 'New setlist name';
  const addButton = createButton('Validate', 'library-small-action');
  const cancelButton = createButton('Cancel', 'library-small-action');
  const submit = () => {
...
window.addEventListener('resize', () => {
  document.querySelectorAll<HTMLElement>('.library-chart-link').forEach(updateChartEntrySubtitleVisibility);
});
