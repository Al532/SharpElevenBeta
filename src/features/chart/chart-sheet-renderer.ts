...
import {
  createAlternateLaneGroups,
  createChartGridLayoutModel,
  getAlternateDisplayTokens,
  getBarBodyLayout,
  getMainDisplayTokens,
  isAlternateChordTokenKind,
  isGraphicSpacerBar,
  isRepeatTokenKind,
  isSlashMarkerTokenKind
} from './chart-sheet-layout-model.js';
import {
  clampNumber,
  clampTokenOffset,
  getRowResizeWeightsFromFactors,
  getWidthFactorFromCompressionScale,
  solveGlobalTokenOffsets
} from './chart-layout-solvers.js';
...
function applyRowBarResizing() {
  ...
  const maxSpreadFactor = Math.max(1, CHART_BAR_RESIZING_CONFIG.maxAppliedDeltaRatio);
  if (maxSpreadFactor <= 1) {
...
function renderBarCell(bar, options: RenderBarCellOptions = {}) {
  ...
  function renderSheet(viewModel) {
    ...
    const layoutModel = createChartGridLayoutModel(
      viewModel,
      barTimeSigDisplay,
      getDisplayedBarGroupSize?.()
    );
    const rows = layoutModel.rows.map((row) => {
      const cells = row.cells.map((cell) => {
        if (cell.kind === 'empty') return renderEmptyBarCell(cell.slotSpan);
        if (cell.kind === 'emptyMeasure') return renderEmptyMeasureCell(cell.slotSpan);
        return renderBarCell(cell.bar, {
          isRowStart: cell.isRowStart,
          sectionChanged: cell.sectionChanged
        });
      });

      return `
        <div class="chart-row${row.sectionChanged ? ' has-section-marker' : ''}" style="--chart-row-columns: ${row.rowColumnCount};">
          ${row.rowTimeSignature ? renderBarTimeSignature(row.rowTimeSignature) : ''}
          <div class="chart-section-marker">
            ${row.sectionChanged ? `<span class="chart-section-badge">${row.firstBar.sectionLabel}</span>` : '<span class="chart-section-spacer"></span>'}
          </div>
          ${cells.join('')}
        </div>
      `;
